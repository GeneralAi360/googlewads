...
class ReviewReadinessTests(unittest.TestCase):
    ...
    def fixture(self, root: Path, count: int = 2):
        rows, files = [], []
        identity = {
            "campaign_design_system_id": "CDS-REVIEW-1", "campaign_design_system_sha256": "1" * 64,
            "idea_architecture_id": "IDEA-REVIEW-1", "visual_character_signature_id": "VC-REVIEW-1", "lighting_intent_id": "LIGHT-REVIEW-1",
        }
        for index in range(1, count + 1):
            ...
            files.append({"job_id": job, "concept_id": "C01", "variant_id": f"V{index:02d}", "language": "ru", "path": output.as_posix(), "sha256": digest, "width": 300, "height": 250, "status": "PASS", **identity})
        matrix = {"run_id": "review-demo", "expected_output_files": count, "banner_matrix": rows}
        manifest = {"campaign_id": "review-demo", "files": files, **identity}
    ...
    def banner_review(self, item, independent=True, status="PASS"):
        ...
        checks = {name: value for name in (
            "concept_fidelity", "idea_fidelity", "emotional_fidelity", "visual_character_fidelity", "campaign_design_system_fidelity",
            "brand_fidelity", "asset_quality", "professional_category_fit", "visual_hierarchy", "lighting_intent_fidelity",
            "lighting_focal_guidance", "typography_legibility", "color_contrast", "information_density", "crop_safe_zones",
            "cta_clarity", "anti_generic_ai", "actual_size_check",
        )}
        return {
            "job_id": item["job_id"], "reviewer_role": "DESIGN_REVIEWER", "independent_context": independent,
            "reviewed_output_path": item["path"], "reviewed_output_sha256": item["sha256"], "status": status,
    ...
    def pack_review(self, manifest_path, independent=True, status="PASS"):
        value = "PASS" if status == "PASS" else "FAIL"
        checks = {name: value for name in (
            "all_expected_files_present", "no_unintended_duplicates", "cross_size_concept_consistency", "idea_consistency",
            "emotional_consistency", "visual_character_consistency", "campaign_design_system_consistency", "brand_consistency",
            "professional_category_consistency", "asset_quality_consistency", "cross_size_lighting_intent_consistency",
            "intentional_layout_adaptation", "small_format_simplification", "contact_sheet_review",
        )}
        return {
            "reviewer_role": "PACK_REVIEWER", "independent_context": independent,
            "manifest_sha256": self.assessor.sha256_file(manifest_path), "status": status,
            "checks": checks, "findings": [],
        }
    ...
    def test_materializer_creates_semantic_lighting_review_tasks(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["campaign_design_system_id"], "CDS-REVIEW-1")
            task = Path(result["banner_reviews"][0]["task_path"]).read_text(encoding="utf-8")
            self.assertIn("idea fidelity", task)
            self.assertIn("emotional fidelity", task)
            self.assertIn("visual-character fidelity", task)
            self.assertIn("lighting-intent fidelity", task)
            pack_task = (root / "review-run" / "pack-review-task.md").read_text(encoding="utf-8")
            self.assertIn("lighting-intent", pack_task)

    def test_materializer_rejects_file_level_design_identity_drift(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            matrix, manifest, manifest_path = self.fixture(root, count=1)
            manifest["files"][0]["lighting_intent_id"] = "LIGHT-DRIFT"
            with self.assertRaises(self.materializer.ReviewMaterializeError) as ctx:
                self.materializer.materialize_reviews(matrix, manifest, root / "review-run", manifest_path=manifest_path)
            self.assertIn("lighting_intent_id", str(ctx.exception))
    ...
    def test_materializer_attaches_hash_bound_design_qa_views(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            qa_root = root / "qa"
            qa_root.mkdir()
            views = {"actual": item["path"], "grayscale": (qa_root / "grayscale.png").as_posix(), "squint": (qa_root / "squint.png").as_posix(), "thumbnail_board": (qa_root / "thumbnail-board.png").as_posix()}
            for name, path in views.items():
                if name != "actual":
                    Path(path).write_bytes(b"diagnostic")
            qa_index = {"artifact_role": "DESIGN_QA_DIAGNOSTICS_ONLY", "delivery_asset": False, "jobs": [{"job_id": item["job_id"], "source_sha256": item["sha256"], "views": views}]}
            qa_path = root / "design-qa-index.json"
            qa_path.write_text(json.dumps(qa_index), encoding="utf-8")
            result = self.materializer.materialize_reviews(matrix, manifest, root / "review-run", manifest_path=manifest_path, qa_index_path=qa_path)
    ...
    def test_materializer_rejects_stale_design_qa_views(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            item = manifest["files"][0]
            qa_path = root / "design-qa-index.json"
            qa_path.write_text(json.dumps({"jobs": [{"job_id": item["job_id"], "source_sha256": "0" * 64, "views": {"actual": item["path"], "grayscale": "g.png", "squint": "s.png", "thumbnail_board": "t.png"}}]}), encoding="utf-8")
            with self.assertRaises(self.materializer.ReviewMaterializeError) as ctx:
                self.materializer.materialize_reviews(matrix, manifest, root / "review-run", manifest_path=manifest_path, qa_index_path=qa_path)
    ...
    def test_lighting_intent_failure_blocks_completion(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            matrix, manifest, manifest_path = self.fixture(root, count=1)
            review_dir, pack_path = self.write_passing_reviews(root, manifest, manifest_path)
            ...
            review["checks"]["lighting_intent_fidelity"] = "FAIL"
            review["status"] = "FAIL"
